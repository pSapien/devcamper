...
const BootcampBadRequest = (id) => new BadRequest(`Bootcamp of ${id} not found`);
...
async function updateBootcamp(req, res, next) {
  const { id } = req.params;
  const bootcamp = await Schema.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
